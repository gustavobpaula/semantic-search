...
- Execução do banco de dados: Docker e Docker Compose, utilizando o `docker-compose` fornecido no repositório de exemplo
...
Este desafio não fixa modelos. Nomes e versões mudam com frequência e alguns são descontinuados, então faz parte do desafio consultar a documentação oficial do provedor escolhido, verificar quais modelos estão disponíveis no momento e selecionar os que atendem ao objetivo.

Para o volume deste desafio, os modelos mais leves e baratos de cada provedor são suficientes.

> **Atenção:** modelos de embedding diferentes geram vetores com dimensões diferentes. A tabela de vetores é criada na primeira ingestão, já com a dimensão do modelo escolhido. Se o modelo de embeddings for trocado posteriormente, a ingestão passa a falhar por incompatibilidade de dimensão. Nesse caso, é necessário apagar a collection existente, ou o volume do banco, e refazer a ingestão do zero com o novo modelo.
...
Faça um fork do [repositório de exemplo](https://github.com/devfullcycle/mba-ia-desafio-ingestao-busca) para utilizar a estrutura abaixo:
...
A entrega deve informar a URL completa do repositório público, incluindo `https://`.
